...
const Details = () => {
    ...
    const add_card = () => {
        if (userInfo) {
            const selectedAddonOptions = Object.values(selectedAddons).filter(option => option); 
    ...
    const handleAddonChange = (addonIndex, option) => {
        setSelectedAddons((prev) => ({
            ...prev,
            [addonIndex]: option, // Save selected option for this addon
        }));
    };

    const calculatePrice = () => {
        let basePrice = product.discount !== 0 ? product.price - Math.floor((product.price * product.discount) / 100) : product.price;
        let addonsPrice = Object.values(selectedAddons).reduce((total, option) => total + (option ? option.price : 0), 0);
        console.log(basePrice+addonsPrice);
        finalPrice=basePrice+addonsPrice;
        // setcardPrice(basePrice+addonsPrice);
    ...
    return (
        <div>
            <Headers />
            <div className='bg-[url("http://142.93.71.196:3000/images/banner/order.jpg")] h-[220px] mt-6 bg-cover bg-no-repeat relative bg-left'>
                        ...
                        <div className='flex flex-col gap-5'>
                            ...
                            <div className='text-2xl text-red-500 font-bold flex gap-3'>
                            {
                                    product.discount !== 0 ? <>
                                        ...
                                        <h2>${calculatePrice()} (-{product.discount}%)</h2>
                                    </> : <h2>Price : ${calculatePrice()}</h2>
                                }
                            </div>
                            ...
                            {product.addons && product.addons.map((addon, index) => (
                                <div key={index} className='flex flex-col gap-2'>
                                    <label className='font-semibold'>{addon.question}</label>
                                    <select 
                                        onChange={(e) => handleAddonChange(index, addon.options.find(option => option.text === e.target.value))}
